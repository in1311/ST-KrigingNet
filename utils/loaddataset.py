...
class DataSet():
    ...
    def scaler_data(self):
        ...
        ##### scale spatiotemporal coordinates x, y, t #####
        data_train_scaler.iloc[:,1:4] = self.scaler_coods.fit_transform(self.train_data.values[:,1:4])  # fit and transform
        data_test_scaler.iloc[:, 1:4] = self.scaler_coods.transform(self.test_data.values[:, 1:4])  # transform

        ##### scale auxiliary variables #####
        data_train_scaler.iloc[:,4:-1] = self.scaler_features.fit_transform(self.train_data.values[:,4:-1])  # fit and transform
        data_test_scaler.iloc[:,4:-1] = self.scaler_features.transform(self.test_data.values[:,4:-1])  # transform
