...
class DIAGNOSIS():
    ...
    def mape(self, outs, labels):
        ...
        mask = abs(labels-0) > 1e-4  #  avoid the denominator to be zero (GSOD data)
...
def tid_to_datetime(tid,time_interval=1):
    start_datetime = datetime.datetime.strptime("2018-01-01","%Y-%m-%d")
    datetime_id = start_datetime + datetime.timedelta(days=tid*time_interval)
    datetime_str = datetime_id.timetuple()
    datetime_str = str(datetime_str.tm_mon) + '/' + str(datetime_str.tm_mday)
    return datetime_str
